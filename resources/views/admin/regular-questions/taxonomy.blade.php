...
@section('title', 'Regular Question Bank — Categories & Types')
...
<div class="bg-white rounded-2xl border border-border p-6 mb-6">
    ...
    <form method="POST" action="{{ route('admin.regular-questions.taxonomy.categories.store') }}" class="flex items-end gap-3">
        @csrf
        <div class="flex-1">
...
<div class="space-y-4">
    @foreach($categories as $category)
        <div class="bg-white rounded-2xl border border-border p-6">
            ...
            <div class="flex flex-wrap gap-2 mb-4">
                @forelse($category->types as $type)
                    <span class="inline-flex items-center gap-2 bg-bg-light border border-border rounded-full px-3 py-1 text-xs text-gray-700">
                        {{ $type->name }}
                        <form method="POST" action="{{ route('admin.regular-questions.taxonomy.types.destroy', $type) }}"
            ...
            <form method="POST" action="{{ route('admin.regular-questions.taxonomy.types.store', $category) }}" class="flex items-end gap-3">
                @csrf
                <div class="flex-1">
                    <input type="text" name="name" placeholder="e.g. 1 Digit - 5 Rows" required
