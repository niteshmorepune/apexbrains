...
@section('title', 'Edit Question — Regular Bank')
@section('page-title', 'Regular Question Editor')
...
<div class="grid grid-cols-1 lg:grid-cols-3 gap-6"
     x-data="{
        ...
        answerFormat: '{{ old('answer_format', $question->answer_format) }}',
     }">
    <div class="col-span-2">
        <form method="POST" action="{{ route('admin.regular-questions.update', $question) }}">
            @csrf @method('PUT')
            ...
            <div class="bg-white rounded-2xl border border-border p-6 mb-4">
                ...
                <div class="grid grid-cols-1 sm:grid-cols-4 gap-4 mb-4">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Answer Format <span class="text-red-500">*</span></label>
                        <select name="answer_format" x-model="answerFormat" required
                                class="w-full border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-fran">
            ...
            <div class="bg-white rounded-2xl border border-border p-6 mb-4" x-show="answerFormat === 'mcq'">
                        ...
                        <div class="flex items-center gap-3">
                            ...
                            <label class="flex items-center gap-1.5 text-sm text-gray-600 cursor-pointer">
                                <input type="radio" name="correct_answer" value="{{ $label }}"
                                       {{ old('correct_answer', strtoupper((string) $question->correct_answer)) === $label ? 'checked' : '' }} class="accent-fran">
                                Correct
                            </label>
                        </div>
                    @endforeach
