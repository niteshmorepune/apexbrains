...
@section('title', 'Regular Question Bank')
@section('page-title', 'Regular Question Bank')

@section('page-actions')
    <div class="flex items-center gap-2">
        <a href="{{ route('admin.regular-questions.taxonomy') }}"
           class="px-4 py-2 border border-border rounded-xl text-sm text-gray-600 hover:bg-bg-light transition-colors">
            Categories &amp; Types
        </a>
        <a href="{{ route('admin.regular-questions.import') }}"
           class="px-4 py-2 border border-border rounded-xl text-sm text-gray-600 hover:bg-bg-light transition-colors">
            Bulk Import
        </a>
        <a href="{{ route('admin.regular-questions.create') }}"
...
<div class="bg-white rounded-2xl border border-border p-5 sticky top-5">
    <form method="GET" action="{{ route('admin.regular-questions.index') }}" id="filterForm">
...
<div class="bg-white rounded-2xl border border-border overflow-hidden">
    <div class="flex border-b border-border">
        @foreach(['all' => 'All Questions', 'mcq' => 'MCQ', 'audio' => 'Audio'] as $key => $label)
            <a href="{{ route('admin.regular-questions.index', array_merge(request()->except('tab', 'page'), ['tab' => $key])) }}"
    ...
    <div class="overflow-x-auto"><table class="w-full min-w-[640px] text-sm">
        <thead>
            <tr class="bg-admin">
                ...
                <th class="text-center px-4 py-3 text-xs font-semibold text-white">Format</th>
        ...
        <tbody class="divide-y divide-border">
                ...
                <tr class="hover:bg-bg-light {{ $q->status === 'pending' ? 'bg-yellow-50' : '' }}">
                    ...
                    <td class="px-4 py-3">
                        <div class="flex items-center justify-center gap-2">
                            <a href="{{ route('admin.regular-questions.edit', $q) }}" class="text-xs text-fran hover:underline">Edit</a>
            ...
            @empty
                <tr><td colspan="6" class="px-5 py-10 text-center text-gray-400">No questions found. Add your first question or bulk import them.</td></tr>
            @endforelse
        </tbody>
    </table></div>
