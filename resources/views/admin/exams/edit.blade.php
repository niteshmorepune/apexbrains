        ...
        <form id="exam-edit-form" method="POST" action="{{ route('admin.exams.update', $exam) }}" class="space-y-5">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    ...
                    <input type="number" name="max_attempts" value="{{ old('max_attempts', $exam->max_attempts) }}" min="1" max="255" placeholder="Unlimited"
                           class="w-full border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-fran">
                </div>
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Scheduled Date/Time (IST)</label>
                    <input type="datetime-local" name="scheduled_at"
                           value="{{ old('scheduled_at', $exam->scheduled_at_ist?->format('Y-m-d\TH:i')) }}"
                           class="w-full border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-fran">
                </div>
            </div>
            ...
            <div>
                ...
                <input type="datetime-local" name="expires_at"
                       value="{{ old('expires_at', $exam->expires_at_ist?->format('Y-m-d\TH:i')) }}"
                       class="w-full border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-fran">
            </div>
