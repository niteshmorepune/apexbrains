...
<div class="px-4 pb-4 space-y-4">
    ...
    <div class="grid grid-cols-3 gap-3">
        <div class="bg-white rounded-2xl border border-border p-3 text-center">
            <p class="text-xl font-black text-fran">{{ $totalDone }}/{{ $totalPapers }}</p>
            <p class="text-[11px] text-gray-400 mt-0.5">Papers Done</p>
        </div>
        <div class="bg-white rounded-2xl border border-border p-3 text-center">
        ...
        <div class="bg-white rounded-2xl border border-border p-3 text-center">
            <p class="text-xl font-black text-logo-amber">{{ round($totalDone / max($totalPapers, 1) * 100) }}%</p>
            <p class="text-[11px] text-gray-400 mt-0.5">Progress</p>
        </div>
    </div>
    ...
    <div>
        ...
        <div class="space-y-3">
            @forelse($attempts as $i => $attempt)
                <a href="{{ route('external.practice.result', $attempt->paper) }}" class="block bg-white rounded-2xl border border-border overflow-hidden">
                    <div class="flex items-stretch">
                        ...
                        <div class="flex items-center gap-3 p-4 flex-1 min-w-0">
                            <span class="w-10 h-10 rounded-full bg-bg-light flex items-center justify-center flex-shrink-0 text-xs font-bold text-fran">{{ $attempt->paper?->paper_number ?? '#' }}</span>
                            <div class="flex-1 min-w-0">
                                <p class="text-sm font-bold text-gray-800 truncate">{{ $attempt->paper?->title ?? 'Practice Paper' }}</p>
                                <p class="text-xs text-gray-400">{{ $attempt->submitted_at?->format('d M Y') }}</p>
                            </div>
                            <div class="text-right flex-shrink-0">
                                <span class="text-sm font-bold {{ $attempt->percentage >= 75 ? 'text-stu' : 'text-logo-amber' }}">{{ number_format($attempt->percentage, 0) }}%</span>
