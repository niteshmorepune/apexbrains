...
<div class="px-4 pb-4 space-y-4">
    ...
    <div class="grid grid-cols-2 gap-3">
        <div class="bg-white rounded-2xl border border-border p-3 text-center">
            ...
            <p class="text-[11px] text-gray-400 mt-0.5">Attempts Done</p>
        </div>
        <div class="bg-white rounded-2xl border border-border p-3 text-center">
    ...
    <div>
        ...
        <div class="space-y-3">
            @forelse($attempts as $i => $attempt)
                <a href="{{ route('external.practice.result', $attempt) }}" class="block bg-white rounded-2xl border border-border overflow-hidden">
                    <div class="flex items-stretch">
                        ...
                        <div class="flex items-center gap-3 p-4 flex-1 min-w-0">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="text-xs text-gray-400">{{ $attempt->submitted_at_ist?->format('d M Y') }} · {{ (int) $attempt->score }}/{{ count($attempt->question_ids ?? []) }}</p>
                            </div>
            ...
            @empty
                <div class="bg-white rounded-2xl border border-border p-10 text-center text-gray-400">
                    ...
                    <p class="text-sm mt-1">Complete a practice attempt to see results here.</p>
                </div>
            @endforelse
        </div>
    </div>
