...
<div class="grid grid-cols-3 gap-6">
    ...
    {{-- Compose --}}
    <div class="space-y-4">
        <div class="bg-white rounded-2xl border border-border p-6" x-data="{ target: 'all' }">
                    ...
                    <div x-show="target === 'level'">
                        ...
                        <select name="level_id" class="w-full border border-border rounded-xl px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-fran">
                            ...
                            @foreach($levels as $level)
                                <option value="{{ $level->id }}">Level {{ $level->number }}</option>
                            @endforeach
                        </select>
                    </div>
            ...
            @forelse($history as $log)
                <div class="px-5 py-4 hover:bg-bg-light">
                    <div class="flex items-start justify-between gap-4">
                        <div class="flex-1">
                            <p class="text-sm text-gray-800">{{ $log->entity_type }}</p>
