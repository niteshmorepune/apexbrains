...
**Per the 2026-08-09 color-scheme update, the shared Student Portal chrome (bottom nav active-state, header) and the Dashboard/Regular Practice/Competition Practice/Level-Up Exam sections are BLUE (`bg-fran`/`text-fran`), not green.** The real Competition Exam flow (`student/competitions/{index,show,attempt,result}.blade.php`, distinct from Competition *Practice*) uses its own brown accent token (`bg-comp`/`text-comp`/`bg-comp-light`, `#8B4A2B` family, defined in `resources/css/app.css`). Results/Profile/Certificate Vault and other unlisted student screens still use the legacy green (`stu`) accent internally, pending a future pass — **green (`stu`) is reserved app-wide as the universal pass/fail·correct/wrong·score-quality semantic color** (e.g. PASSED badges, Correct/Wrong tiles, accuracy tiers) and must stay green even on blue- or brown-themed pages; never repurpose it as a generic brand accent. Student screens sit on a light page background (`bg-stu-bg` — a neutral token despite the name) with a per-screen back-chevron + centered title via `<x-student-header>`. Do not reintroduce a persistent colored top app-bar.
