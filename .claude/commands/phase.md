...
- **Responsive pass:** Admin + Franchise portals are now fully responsive (off-canvas drawer below `lg`, responsive grids, horizontal-scroll tables). Student + External were already mobile-first. See the "Responsive layout system" entry in `memory/feedback_patterns.md` for the conventions to follow on any new admin/franchise screen.
...
3. Any open follow-ups from `memory/project_overview.md` → "Known remaining QA items" (e.g. Student/External not yet widened for large desktop screens).
